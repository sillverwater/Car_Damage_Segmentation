...
def main(user_img):
    labels = ['Breakage', 'Crushed', 'Seperated', 'Scratch']
    labels_k = ['파손', '찌그러짐', '이격', '스크래치']
    models = []
    ...
    for i, label in enumerate(labels):
        ...
        if (area!=0):
            damage_list = damage_list + labels_k[i] + ' '
            ...
            cv2.imwrite('./static/result/' + label + '_line.jpg', line_img)
    ...
    damage_message ='( ' + damage_list + ') ' + '이(가) 검출되었습니다'

    return output_path, damage_message
